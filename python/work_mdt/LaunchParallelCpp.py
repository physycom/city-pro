...
if __name__ == "__main__":
    ...
    command_set_right_values_config = "python3 ./python/work_mdt/SetRightDirectoriesConfiguration.py"
    run_adjust_configuration_file(command_set_right_values_config)
#    build(build_command)
    with Pool(len(commands)) as pool:
        pool.map(run_command, commands)

    cmd_python_analysis = "python3 ./python/work_mdt/AnalysisPaper.py ./config"
